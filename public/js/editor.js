...
function drawImage(editor) {
  var cm = editor.codemirror;
  var stat = getState(cm);
  var txtzone = document.querySelector('.CodeMirror');
  ...
  var oForm = '<div id="uploadImg" class="mdl-card mdl-shadow--2dp">'+
              '<div class="mdl-card__title">'+
                '<h2 class="mdl-card__title-text">图片上传</h2>'+
              '</div>'+
              '<div class="mdl-card__supporting-text">'+
                '<div class="mdl-textfield mdl-js-textfield">'+
                  '<input type="file" name="imgFile" class="mdl-textfield__input" id="imgFile">'+
                '</div>'+
              '</div>'+
              '<div class="mdl-card__actions mdl-card--border">'+
                '<button type="button" id="imgSubmit" class="mdl-button mdl-button--accent mdl-js-button mdl-js-ripple-effect">上传</button>'+
              '</div>'+
              '<div class="mdl-card__menu">'+
                '<button id="closeMask" class="mdl-button mdl-button--icon mdl-js-button mdl-js-ripple-effect mdl-button--colored">'+
                  '<i class="material-icons">clear</i>'+
                '</button>'+
              '</div>'+
            '</div>';

  txtzone.appendChild(maskDiv);

  document.querySelector('#huyMask').innerHTML = oForm;

  var myform = document.querySelector('#imgSubmit');
  myform.onclick = function(){
    //模拟表单
    function createForm(data){
      var formElement = document.getElementById('virtualForm');
      var oMyForm = new FormData(formElement);
      for(var i in data){
        oMyForm.append(i, data[i]);
      }
      return oMyForm;
    } 

    var loading = document.createElement('div');
    loading.className = 'mdl-spinner mdl-js-spinner is-active';
    ...
    componentHandler.upgradeElement(loading);
    document.querySelector('#uploadImg').appendChild(loading);
    var formData = createForm({
      Token: '4978ed630a6d2eb74999655cddb2ec6990f85f18:anNNdEpBQW5Ec1lYQV9oVUkwc0lWT0N0bzJvPQ==:eyJkZWFkbGluZSI6MTQ1MDA4MzE1NCwiYWN0aW9uIjoiZ2V0IiwidWlkIjoiNTExNDAzIiwiYWlkIjoiMTA2MzYyNiIsImZyb20iOiJmaWxlIn0=',
      file: document.querySelector('#imgFile').files[0]
    });
    var options = {
      method: 'POST',
      ...
      success: function(data){
        ...
        //console.log(data);
        _replaceSelection(cm, stat.image, '![', ']('+ data.s_url +')'); //展示图
        var maskTmp = document.querySelector('#huyMask');
        maskTmp.parentNode.removeChild(maskTmp);
      }
    }
    ajax(options);
    return false;
  ...
  document.getElementById('closeMask').onclick = function(){
    var maskTmp = document.querySelector('#huyMask');
    maskTmp.parentNode.removeChild(maskTmp);
    return false;
  }
...
function addStyle(element, styles){
  if(!element){
    console.log('addStyle params error!');
    return;
  }
  for(i in styles){
